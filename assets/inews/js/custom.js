$(document).ready(function() {
    ...
    if ($('.archive_submit').is(':visible')) {
        $('.archive_submit').click(function() {
            var URL = window.location.origin + '/archive/';
    ...
    var voteBtn = document.getElementById("vote_btn");
    if (voteBtn) {
        voteBtn.addEventListener("click", function() {
            var q_id = document.getElementById("q_id").getAttribute('data-id');
            var radioValue = document.querySelector("input[name='poll_option']:checked").value;
            var url = '/poll/' + q_id;
            var message = document.getElementsByClassName("msg")[0];
            var data = {
                'poll_option': radioValue
            };
            let lhttp = new XMLHttpRequest();
            if (localStorage.getItem(q_id) != true && localStorage.getItem(q_id) == null) {
                lhttp.onreadystatechange = function() {
                    if (this.readyState == 4 && this.status == 200) {
                        localStorage.setItem(q_id, true);
                ...
                lhttp.send(JSON.stringify(data));
            } else {
                message.innerHTML = 'আপনি এর আগেও একবার ভোট দিয়েছেন, তাই এই ভোটটি গৃহীত হলো না।';
            }
        });
    }
    // var footerAd = jQuery('.sticky-footer-ad');
    // if (footerAd) {
    //     jQuery(window).on('scroll', _.debounce(upNdown, 200));
    //     var position = jQuery(window).scrollTop();

    //     function upNdown() {
    //         var scrollTop = jQuery(window).scrollTop();
    //         if (scrollTop > position) {
    //             footerAd.slideUp("slow");
    //         } else {
    //             footerAd.slideDown("slow");
    //         }
    //         position = scrollTop;
    //     }
    // }
